...
func Run(ctx context.Context, c *client.Client, opt Options) (*Result, error) {
	...
	for i, path := range paths {
		...
		err := repairOne(ctx, c, path, opt, logger)
...
func repairOne(ctx context.Context, c *client.Client, path string, opt Options, logger *log.Logger) error {
	...
	if opt.DryRun {
		logger.Printf("DRY-RUN %s: copy→%s, move %s→%s, move %s→%s%s",
			path, tempPath, path, backupPath, tempPath, path,
			map[bool]string{true: ", drop backup", false: ""}[opt.DropBackup])
		return nil
	}
	...
	if opt.DropBackup {
		...
		if err := c.DropTable(ctx, backupPath); err != nil {
			return fmt.Errorf("%s: drop backup: %w", path, err)
		}
	} else {
