...
func Run(ctx context.Context, c *client.Client, opt Options) (*Result, error) {
	...
	for i, path := range paths {
		...
		dropped, err := repairOne(ctx, c, path, opt, logger)
		...
		logger.Printf("%s OK %s", prefix, path)
...
// repairOne repairs one table. Returns whether a backup was dropped.
func repairOne(ctx context.Context, c *client.Client, path string, opt Options, logger *log.Logger) (backupDropped bool, err error) {
	...
	for _, extra := range []string{tempPath, backupPath} {
		...
		if exists {
			return false, fmt.Errorf("%s: helper path already exists: %s", path, extra)
		}
	}
	...
	if opt.DryRun {
		...
		if opt.DropBackup {
			msg += fmt.Sprintf(", drop backup %s", backupPath)
		}
		logger.Printf("%s", msg)
		return opt.DropBackup, nil
	}
	...
	if err := c.CopyWithStorage(ctx, path, parent, tempName, opt.PoolKind); err != nil {
		return false, fmt.Errorf("%s: copy: %w", path, err)
	}
	if err := assertRepaired(ctx, c, tempPath); err != nil {
		return false, fmt.Errorf("%s: temp copy check: %w", path, err)
	}

	logger.Printf("%s: renaming %s → %s", path, path, backupPath)
	if err := c.MoveTable(ctx, path, backupPath); err != nil {
		return false, fmt.Errorf("%s: rename to backup: %w", path, err)
	}

	logger.Printf("%s: renaming %s → %s", path, tempPath, path)
	if err := c.MoveTable(ctx, tempPath, path); err != nil {
		return false, fmt.Errorf("%s: rename temp to original: %w", path, err)
	}
	if err := assertRepaired(ctx, c, path); err != nil {
		return false, fmt.Errorf("%s: post-check: %w", path, err)
	}

	if opt.DropBackup {
		logger.Printf("%s: dropping backup %s", path, backupPath)
		if err := c.DropTable(ctx, backupPath); err != nil {
			return false, fmt.Errorf("%s: drop backup: %w", path, err)
		}
		return true, nil
	}
