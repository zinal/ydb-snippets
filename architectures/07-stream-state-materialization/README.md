...
Yandex Message Queue здесь не требуется. YMQ — отдельная SQS-совместимая очередь поверх YDB для competing consumers, visibility timeout, retry и DLQ; она не является YDB Topic и не предоставляет модель pub/sub log с offset именованных YDB Consumers.
