...
1. Copy the `group_vars/all.example` file into `group_vars/all`, and customize it according to your environment:
   ...
   * `ydb_dir` is the path of the YDB software installation directory to be created on the hosts;
   * `ydb_tls_dir` is the path to the directory with the TLS certificates and keys, as generated by the [sample script](https://github.com/ydb-platform/ydb/tree/main/ydb/deploy/tls_cert_gen), or following the filename convention used by the sample script;
   * `ydb_archive` must point to the YDB server binary package downloaded;
   * `ydb_unpack_options` sets extra flags to be passed to `tar` for unpacking the YDB server binaries, typically should contain the value `--strip-component=1`;
   * `ydb_config` shoud be the name of the cluster configuration file within the `files` subdirectory.
1. Copy the `group_vars/ydbd_static.example` file into `group_vars/ydbd_static`, and specify the desired disk layout. `ydb_disks` parameter is the list of structures, each having the following fields:
...
1. Copy the `group_vars/ydbd_dynamic.example` file into `group_vars/ydbd_dynamic`, and specify the desired list of dynamic nodes to be ran on each host:
    * `ydb_dynnodes` parameter is the list of structures, each having the following fields:
    ...
    *  `ydb_brokers` is the list of host names running the YDB static nodes, exactly 3 (three) host names must be specified;
